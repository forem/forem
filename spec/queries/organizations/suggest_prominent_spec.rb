...
RSpec.describe Organizations::SuggestProminent, type: :service do
  ...
  context "when user is following any tags" do
    ...
    it "returns organizations with posts with at least an average score, within the last 3 weeks under tags that the user has followed" do
  ...
  it "returns organizations with posts with at least an average score, within the last 3 weeks under any tags" do
    results = suggester.suggest
    expect(results).to contain_exactly(*top_organizations)
    expect(results).not_to include(bad_organizations.first)
    expect(results).not_to include(bad_organizations.last)
  end
end 
