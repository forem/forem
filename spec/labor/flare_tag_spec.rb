...
RSpec.describe FlareTag, type: :labor do
  ...
  describe "#flare_tag" do
    xit "returns nil if there is no flare tag" do
      expect(described_class.new(article).tag).to be nil
    end

    xit "returns a flare tag if there is a flare tag in the list" do
      valid_article = create(:article, tags: "ama")
      expect(described_class.new(valid_article).tag.name).to eq("ama")
    end

    xit "returns nil if an except is provided" do
    ...
    xit "returns a flare tag if there are 2 flare tags in the list" do
  ...
  describe "#flare_tag_hash" do
    ...
    xit "returns nil if an article doesn't have a flare tag" do
      expect(described_class.new(article).tag_hash).to be nil
    end

    xit "returns a hash with the flare tag's name" do
      expect(described_class.new(valid_article).tag_hash.value?("ama")).to be true
    end

    xit "returns a hash with the flare tag's bg_color_hex" do
      expect(described_class.new(valid_article).tag_hash.value?("#f3f3f3")).to be true
    end

    xit "returns a hash with the flare tag's text_color_hex" do
