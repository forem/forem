...
describe Rack::Attack, type: :request, throttle: true do
  ...
  describe "message_tag_throttle" do
    ...
    it "throttles viewing tags" do
      allow_any_instance_of(TagArticlesController).to receive(:tagged_count).and_return(0)
      allow_any_instance_of(TagArticlesController).to receive(:stories_by_timeframe).and_return(Article.none)
