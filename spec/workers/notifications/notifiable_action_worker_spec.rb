...
RSpec.describe Notifications::NotifiableActionWorker, type: :worker do
  describe "#perform" do
    ...
    xit "calls the service when existing notifiable passed" do
    ...
    xit "doesn't call a service when notifiable doesn't exist" do
      worker.perform(Article.maximum(:id).to_i + 1, "Article", action)
      expect(service).not_to have_received(:call)
    end

    xit "doesn't call a service when unexpected notifiable type passed" do
      user = create(:user)
