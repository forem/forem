...
RSpec.describe Subforems::CreateFromScratchWorker do
  ...
  describe "#perform" do
    ...
    it "sets up the subforem with all AI services" do
      described_class.new.perform(subforem.id, brain_dump, name, logo_url, bg_image_url)
    ...
    it "works without background image URL" do
      described_class.new.perform(subforem.id, brain_dump, name, logo_url)
    ...
    context "when an error occurs" do
      before do
        allow(Settings::Community).to receive(:set_community_name).and_raise(StandardError, "Settings error")
