...
RSpec.describe "ArticlesApi", type: :request do
  ...
  let(:secret) { create(:api_secret, user: user1) }
  ...
  describe "POST /api/articles w/ api key" do
    it "creates ordinary article with proper params" do
      new_title = "NEW TITLE #{rand(100)}"
      post "/api/articles", params: {
        article: { title: new_title, body_markdown: "Yo ho ho#{rand(100)}", tag_list: "yo", key: api_secret.secret }
      }
      expect(Article.last.user_id).to eq(user1.id)
    end
  ...
  describe "PUT /api/articles/:id w/ api key" do
    let(:article) { create(:article, user: user1) }
    ...
    it "does not allow user to update a different article" do
      ...
      invalid_update_request = -> do
        put "/api/articles/#{article.id}", params: {
          article: { title: "NEW TITLE #{rand(100)}",
                     body_markdown: "Yo ho ho#{rand(100)}", tag_list: "yo",
                     key: api_secret.secret }
        }
      end
    ...
    it "allows collection to be assigned via api" do
      ...
      collection = Collection.create(user_id: article.user_id, slug: "yoyoyo")
      put "/api/articles/#{article.id}", params: {
        article: { title: new_title, body_markdown: "Yo ho ho#{rand(100)}", tag_list: "yo", collection_id: collection.id, key: api_secret.secret }
      }
      expect(Article.last.collection_id).to eq(collection.id)
    end
