...
RSpec.shared_examples "redirects to the lowercase route" do
  context "when a path contains uppercase characters" do
    xit "redirects to the lowercase route" do
      get path
...
RSpec.describe "StoriesIndex", type: :request do
  describe "GET stories index" do
    xit "renders page with article list" do
    ...
    xit "renders proper description" do
      get "/"
    ...
    xit "renders left display_ads when published and approved" do
    ...
    xit "renders right display_ads when published and approved" do
      org = create(:organization)
    ...
    xit "does not render left display_ads when not approved" do
      org = create(:organization)
    ...
    xit "does not render right display_ads when not approved" do
      org = create(:organization)
    ...
    xit "has gold sponsors displayed" do
      org = create(:organization)
    ...
    xit "does not display silver sponsors" do
      org = create(:organization)
    ...
    xit "does not display non live gold sponsorships" do
      org = create(:organization)
    ...
    xit "shows default meta keywords" do
    ...
    context "with campaign hero" do
      ...
      xit "displays hero html when it exists and is set in config" do
      ...
      xit "doesn't display when campaign_hero_html_variant_name is not set" do
      ...
      xit "doesn't display when hero html is not approved" do
        SiteConfig.campaign_hero_html_variant_name = "hero"
    ...
    context "with campaign_sidebar" do
      ...
      xit "displays unapproved post if approval is not required" do
        SiteConfig.campaign_sidebar_enabled = true
        get "/"
        expect(response.body).to include(CGI.escapeHTML("Unapproved-post"))
      end

      xit "displays only approved posts with the campaign tags" do
        SiteConfig.campaign_sidebar_enabled = false
        get "/"
        expect(response.body).not_to include(CGI.escapeHTML("Super-puper"))
      end

      xit "displays sidebar url if campaign_url is set" do
        SiteConfig.campaign_sidebar_enabled = true
      ...
      xit "does not display sidebar url if image is not present is set" do
        SiteConfig.campaign_sidebar_enabled = true
        SiteConfig.campaign_url = "https://campaign-lander.com"
        get "/"
  ...
  describe "GET query page" do
    xit "renders page with proper header" do
      get "/search?q=hello"
  ...
  describe "GET tag index" do
    ...
    xit "renders page with top/week etc." do
    ...
    xit "renders tag after alias change" do
    ...
    xit "does not render sponsor if not live" do
      sponsorship = create(
    ...
    context "without user signed in" do
      ...
      xit "does not include sidebar for page tag" do
        create_list(:article, 20, user: user, featured: true, tags: [tag.name], score: 20)
        get "/t/#{tag.name}/page/2"
        expect(response.body).not_to include('<div id="sidebar-wrapper-right"')
      end

      xit "does not include current page link" do
        create_list(:article, 20, user: user, featured: true, tags: [tag.name], score: 20)
        get "/t/#{tag.name}/page/2"
      ...
      xit "renders proper canonical url for page 1" do
        get "/t/#{tag.name}"
        expect(response.body).to include("<link rel=\"canonical\" href=\"http://localhost:3000/t/#{tag.name}\" />")
      end

      xit "renders proper canonical url for page 2" do
        get "/t/#{tag.name}/page/2"
