...
RSpec.describe "StoriesShow", type: :request do
  ...
  describe "GET /:username/:slug (articles)" do
    xit "renders proper title" do
      get article.path
      expect(response.body).to include CGI.escapeHTML(article.title)
    end

    xit "redirects to appropriate if article belongs to org and user visits user version" do
    ...
    ## Title tag
    xit "renders signed-in title tag for signed-in user" do
      sign_in user
      get article.path
      expect(response.body).to include "<title>#{CGI.escapeHTML(article.title)} - #{community_qualified_name} 👩‍💻👨‍💻</title>"
    end

    xit "renders signed-out title tag for signed-out user" do
      get article.path
    ...
    xit "renders title tag with search_optimized_title_preamble if set and not signed in" do
    ...
    xit "does not render title tag with search_optimized_title_preamble if set and not signed in" do
      sign_in user
      article.update_column(:search_optimized_title_preamble, "Hey this is a test")
      get article.path
      expect(response.body).to include "<title>#{CGI.escapeHTML(article.title)} - #{community_qualified_name} 👩‍💻👨‍💻</title>"
    ...
    xit "does not render preamble with search_optimized_title_preamble not signed in but search_optimized_title_preamble not set" do
      get article.path
      expect(response.body).to include "#{CGI.escapeHTML(article.title)} - #{community_name}</title>"
    end

    xit "renders title preamble with search_optimized_title_preamble if set and not signed in" do
      article.update_column(:search_optimized_title_preamble, "Hey this is a test")
      get article.reload.path
      expect(response.body).to include "<span class=\"article-title-preamble\">Hey this is a test</span>"
    end

    xit "does not render preamble with search_optimized_title_preamble if set and signed in" do
      sign_in user
      article.update_column(:search_optimized_title_preamble, "Hey this is a test")
      get article.path
    ...
    xit "does not render title tag with search_optimized_title_preamble not signed in but search_optimized_title_preamble not set" do
      get article.path
      expect(response.body).not_to include "<span class=\"article-title-preamble\">Hey this is a test</span>"
    end

    xit "renders second and third users if present" do
    ...
    xit "renders articles of long length without breaking" do
    ...
    xit "redirects to appropriate page if user changes username" do
    ...
    xit "redirects to appropriate page if user changes username twice" do
      old_username = user.username
      user.update(username: "new_hotness_#{rand(10_000)}")
    ...
    xit "redirects to appropriate page if user changes username twice and go to middle username" do
      user.update(username: "new_hotness_#{rand(10_000)}")
    ...
    xit "has noindex if article has low score" do
      article = create(:article, score: -5)
      get article.path
      expect(response.body).to include("noindex")
    end

    xit "has noindex if article has low score even with <code>" do
      article = create(:article, score: -5)
    ...
    xit "does not have noindex if article has high score" do
      article = create(:article, score: 6)
      get article.path
      expect(response.body).not_to include("noindex")
    end

    xit "does not have noindex if article intermediate score and <code>" do
    ...
    xit "does not have noindex if article w/ intermediate score w/ 1 comment " do
      article = create(:article, score: 3)
  ...
  describe "GET /:username (org)" do
    xit "redirects to the appropriate page if given an organization's old slug" do
    ...
    xit "redirects to the appropriate page if given an organization's old old slug" do
      original_slug = org.slug
      org.update(slug: "somethingnew")
