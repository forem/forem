...
RSpec.shared_examples "an elevated privilege required request" do |path|
  context "when not logged-in" do
    xit "does not grant acesss", proper_status: true do
      get path
      expect(response).to have_http_status(:not_found)
    end

    xit "raises Pundit::NotAuthorizedError internally" do
  ...
  context "when user is not trusted" do
    ...
    xit "internally raise Pundit::NotAuthorized internally" do
      expect { get path }.to raise_error(Pundit::NotAuthorizedError)
    end
  end
end
