...
RSpec.describe "ClassifiedListings", type: :request do
  ...
  describe "GET /listings" do
    ...
    xit "returns text/html and has status 200" do
      get "/listings"
    ...
    context "when the user has no params" do
      xit "shows all active listings" do
        get "/listings"
    ...
    context "when the user has category and slug params for active listing" do
      xit "shows that direct listing" do
    ...
    context "when the user has category and slug params for expired listing" do
      xit "shows only active listings from that category" do
    ...
    context "when view is moderate" do
      xit "redirects to internal/listings/:id/edit" do
      ...
      xit "without a slug raises an ActiveRecord::RecordNotFound error" do
        expect do
  ...
  describe "GET /listings/new" do
    ...
    context "when the user has credits" do
      xit "shows the number of credits" do
    ...
    context "when the user has no credits and belongs to an organization" do
      ...
      xit "shows the proper message when both user and org have no credits" do
        get "/listings/new"
        expect(response.body).to include "Listings Require Credits"
      end

      xit "shows the number of credits of the user if the user has credits but the org has no credits" do
        random_number = rand(2..100)
        create_list(:credit, random_number, user: user)
        get "/listings/new"
        expect(response.body).to include "You have #{random_number} credits available"
      ...
      xit "shows the number of credits of the organization if the org has credits" do
        random_number = rand(2..100)
      ...
      xit "shows the number of credits of both the user and the organization if they both have credits" do
        random_number = rand(2..100)
        create_list(:credit, random_number, organization: organization)
        create_list(:credit, random_number, user: user)
        get "/listings/new"
  ...
  describe "POST /listings" do
    ...
    context "when the listing is invalid" do
      ...
      xit "renders errors with the listing" do
    ...
    context "when the listing is valid" do
      xit "redirects if the user does not have enough credits" do
        Credit.delete_all
        post "/listings", params: listing_params
        expect(response.body).to redirect_to("/credits")
      end

      xit "redirects if the org does not have enough credits" do
      ...
      xit "redirects to /listings/dashboard for listing draft" do
        post "/listings", params: draft_params
        expect(response).to redirect_to "/listings/dashboard"
      end

      xit "redirects to /listings" do
        post "/listings", params: listing_params
  ...
  describe "PUT /listings/:id" do
    ...
    context "when the bump action is called" do
      ...
      xit "does not bump the user listing and redirects to credits if the user has not enough credits" do
    ...
    context "when the unpublish action is called" do
      ...
      xit "unpublishes a published listing" do
        put "/listings/#{listing.id}", params: params
    ...
    context "when an update is attempted" do
      xit "does not update with an empty body markdown" do
  ...
  describe "DEL /listings/:id" do
    ...
    context "when deleting draft" do
      xit "redirect to dashboard" do
        delete "/listings/#{listing_draft.id}"
        expect(response).to redirect_to("/listings/dashboard")
      end

      xit "have status 302 found" do
        delete "/listings/#{listing_draft.id}"
        expect(response).to have_http_status(:found)
      end

      xit "decrease total listings count by 1" do
        expect do
          delete "/listings/#{listing_draft.id}"
        end.to change(ClassifiedListing, :count).by(-1)
      end
