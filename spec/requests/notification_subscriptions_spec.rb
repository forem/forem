...
RSpec.describe "NotificationSubscriptions", type: :request do
  ...
  describe "#show or GET /notification_subscriptions/:notifiable_type/:notifiable_id" do
    context "when signed in" do
      ...
      xit "returns the correct subscription boolean as JSON" do
        get "/notification_subscriptions/Article/#{article.id}",
            headers: headers
        expect(JSON.parse(response.body)["config"]).to eq "all_comments"
      end

      xit "returns the correct subscription boolean as JSON if unsubscribed" do
    ...
    xit "returns a JSON response 'null' if there is no logged in user" do
  ...
  describe "#upsert or POST /notification_subscriptions/:notifiable_type/:notifiable_id" do
    xit "returns 404 if there is no logged in user" do
      expect do
    ...
    context "when sent as a JSON request with the correct params" do
      ...
      xit "completes a proper subscription" do
      ...
      xit "removes a previous subscription" do
      ...
      xit "updates the article.receive_notifications column correctly if the current_user is the author" do
        post "/notification_subscriptions/Article/#{article.id}",
             headers: headers,
             params: { config: "not_subscribed" }
      ...
      xit "updates the comment.receive_notifications column correctly if the current_user is the commenter" do
    ...
    context "when an article has two parent comments by two different people" do
      ...
      xit "mutes the parent comment" do
      ...
      xit "does not mute the someone else's parent comment" do
      ...
      xit "unmutes the parent comment if already muted" do
    ...
    context "when an article has a single comment thread with multiple commenters" do
      ...
      xit "mutes all of the original commenter's comments in a single thread" do
      ...
      xit "does not mute someone else's comment of a different thread" do
        expect(parent_comment_by_other.receive_notifications).to be true
      end

      xit "does not mute the other commenter's comments in the same thread" do
