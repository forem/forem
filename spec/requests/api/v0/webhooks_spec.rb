...
RSpec.describe "Api::V0::Webhooks", type: :request do
  ...
  describe "GET /api/v0/webhooks" do
    ...
    context "when accessing with oauth" do
      xit "returns a 401 if unauthorized" do
        get api_webhooks_path
        expect(response).to have_http_status(:unauthorized)
      end

      xit "returns a 403 if public scope is missing (oauth)" do
      ...
      xit "returns a 200 if authorized" do
    ...
    context "when accessing via cookie" do
      ...
      xit "returns json on success" do
        get api_webhooks_path
      ...
      xit "returns the correct json representation" do
        get api_webhooks_path
  ...
  describe "GET /api/v0/webhooks/:id" do
    ...
    xit "returns 404 if the webhook does not exist" do
      get api_webhook_path(9999)

      expect(response).to have_http_status(:not_found)
    end

    xit "returns 404 if another user webhook is accessed" do
    ...
    xit "returns the correct json representation for the webhook user" do
      get api_webhook_path(webhook.id)
  ...
  describe "POST /api/v0/webhooks" do
    ...
    xit "returns :created and json response on success" do
      post api_webhooks_path, params: { webhook_endpoint: webhook_params }
  ...
  describe "DELETE /api/v0/webhooks/:id" do
    ...
    xit "deletes the webhook" do
      expect do
        delete api_webhook_path(webhook.id)
      end.to change(Webhook::Endpoint, :count).by(-1)
    end

    xit "returns 204 on success" do
      delete api_webhook_path(webhook.id)
      expect(response).to have_http_status(:no_content)
    end

    xit "doesn't allow to destroy other user webhook" do
      other_webhook = create(:webhook_endpoint, user: create(:user))
      expect do
  ...
  describe "authorized with doorkeeper" do
    ...
    xit "renders index successfully" do
      get api_webhooks_path, params: { access_token: access_token.token }
      expect(response.content_type).to eq("application/json")
      expect(response).to have_http_status(:ok)
    end

    xit "renders only corresponding webhooks" do
    ...
    xit "sets correct oauth app id for the webhook if needed" do
      webhook_params = {
        source: "stackbit",
        target_url: Faker::Internet.url(scheme: "https"),
        events: %w[article_created article_updated article_destroyed]
    ...
    xit "doesn't allow destroying another app webhook" do
