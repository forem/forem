...
RSpec.describe "Api::V0::Articles", type: :request do
  ...
  describe "GET /api/articles" do
    ...
    xit "has correct keys in the response" do
    ...
    context "without params" do
      ...
      xit "returns nothing if params state=all is not found" do
        get api_articles_path(state: "all")
        expect(response.parsed_body.size).to eq(0)
      end

      xit "returns featured articles if no param is given" do
      ...
      xit "returns flare tag in the response" do
        get api_articles_path
      ...
      xit "sets the correct edge caching surrogate key" do
        get api_articles_path
    ...
    context "with username param" do
      xit "returns user's articles for the given username" do
        create(:article, user: article.user)
        get api_articles_path(username: article.user.username)
        expect(response.parsed_body.size).to eq(2)
      end

      xit "returns nothing if given user is not found" do
        get api_articles_path(username: "foobar")
        expect(response.parsed_body.size).to eq(0)
      end

      xit "returns org's articles if org's slug is given" do
    ...
    context "with tag param" do
      xit "returns tag's articles" do
        get api_articles_path(tag: article.tag_list.first)
        expect(response.parsed_body.size).to eq(1)
      end

      xit "returns top tag articles if tag and top param is present" do
        get api_articles_path(tag: article.tag_list.first, top: "7")
        expect(response.parsed_body.size).to eq(1)
      end

      xit "returns not tag articles if article and tag are not approved" do
    ...
    context "with top param" do
      xit "only returns fresh top articles if top param is present" do
    ...
    context "with collection_id param" do
      xit "returns a collection id" do
    ...
    context "with state param" do
      ...
      xit "returns rising articles" do
      ...
      xit "returns nothing if the state is unknown" do
    ...
    context "with regression tests" do
      xit "works if both the social image and the main image are missing" do
  ...
  describe "GET /api/articles/:id" do
    ...
    xit "returns all the relevant datetimes" do
    ...
    xit "fails with an unpublished article" do
      article.update_columns(published: false)
      get api_article_path(article.id)
      expect(response).to have_http_status(:not_found)
    end

    xit "fails with an unknown article ID" do
  ...
  describe "GET /api/articles/me(/:status)" do
    context "when request is unauthenticated" do
      ...
      xit "returns forbidden when requesting for all with only public scope" do
        get me_api_articles_path(status: :all), params: { access_token: public_token.token }
        expect(response).to have_http_status(:forbidden)
      end

      xit "returns forbidden status when requesting unpublished with public scope" do
    ...
    context "when request is authenticated" do
      ...
      xit "works with bearer authorization" do
      ...
      xit "returns proper response specification" do
        get me_api_articles_path, params: { access_token: access_token.token }
        expect(response.content_type).to eq("application/json")
        expect(response).to have_http_status(:ok)
      end

      xit "returns success when requesting publiched articles with public token" do
      ...
      xit "return only user's articles including markdown" do
      ...
      xit "only includes published articles by default" do
        create(:article, published: false, published_at: nil, user: user)
        get me_api_articles_path, params: { access_token: access_token.token }
        expect(response.parsed_body.length).to eq(0)
      end

      xit "only includes published articles when asking for published articles" do
        create(:article, published: false, published_at: nil, user: user)
        get me_api_articles_path(status: :published), params: { access_token: access_token.token }
        expect(response.parsed_body.length).to eq(0)
      end

      xit "only includes unpublished articles when asking for unpublished articles" do
        create(:article, published: false, published_at: nil, user: user)
        get me_api_articles_path(status: :unpublished), params: { access_token: access_token.token }
        expect(response.parsed_body.length).to eq(1)
      end

      xit "orders unpublished articles by reverse order when asking for unpublished articles" do
      ...
      xit "puts unpublished articles at the top when asking for all articles" do
        create(:article, user: user)
        create(:article, published: false, published_at: nil, user: user)
  ...
  describe "POST /api/articles" do
    ...
    describe "when authorized" do
      ...
      xit "returns a 403 if :write_articles scope is missing (oauth)" do
      ...
      xit "returns a 201 if :write_articles scope is provided (oauth)" do
      ...
      xit "returns a 429 status code if the rate limit is reached" do
      ...
      xit "fails if no params are given" do
        post_article
        expect(response).to have_http_status(:unprocessable_entity)
      end

      xit "fails if missing required params" do
      ...
      xit "fails if article contains tags with non-alphanumeric characters" do
        tags = %w[#discuss .help]
        post_article(title: "Test Article Title", tags: tags)
        expect(response).to have_http_status(:unprocessable_entity)
      end

      xit "fails if params are not a Hash" do
      ...
      xit "creates a published article" do
        title = Faker::Book.title
        expect do
      ...
      xit "creates an article with a title and the markdown body" do
      ...
      xit "creates an article with a title, body and a list of tags" do
        tags = %w[meta discussion]
        expect do
          post_article(
            title: Faker::Book.title,
      ...
      xit "creates an unpublished article with the front matter in the body" do
      ...
      xit "creates published article with the front matter in the body" do
      ...
      xit "creates an article within a series" do
        series = "a series"
        post_article(
          title: Faker::Book.title,
          body_markdown: "Yo ho ho",
      ...
      xit "creates article within a series using the front matter" do
      ...
      xit "creates an article on behalf of an organization" do
      ...
      xit "creates an article with a main/cover image" do
      ...
      xit "creates an article with a main/cover image in the front matter" do
        image_url = "https://dummyimage.com/100x100"
      ...
      xit "creates an article with a canonical url" do
      ...
      xit "creates an article with a canonical url in the front matter" do
        canonical_url = "https://example.com/"
      ...
      xit "creates an article with the given description" do
      ...
      xit "creates an article with description in the front matter" do
        description = "this is a very interesting article"
        body_markdown = file_fixture("article_published_canonical_url.txt").read
        expect do
          post_article(
      ...
      xit "creates an article with a part of the body as a description" do
        expect do
          post_article(
            title: Faker::Book.title,
      ...
      xit "does not raise an error if article params are missing" do
        headers = { "api-key" => api_secret.secret, "content-type" => "application/json" }
        expect do
      ...
      xit "fails with a nil body markdown" do
  ...
  describe "PUT /api/articles/:id" do
    ...
    describe "when authorized" do
      ...
      xit "returns a 200 if :write_articles scope is provided (oauth)" do
        access_token = create(:doorkeeper_access_token, resource_owner_id: user.id, scopes: "write_articles")
        headers = { "authorization" => "Bearer #{access_token.token}", "content-type" => "application/json" }
      ...
      xit "returns not found if the article does not belong to the user" do
      ...
      xit "lets a super admin update an article belonging to another user" do
      ...
      xit "does not update title if only given a title because the article has a front matter" do
      ...
      xit "updates the title and the body if given a title and a body" do
        title = Faker::Book.title
        body_markdown = "foobar"
      ...
      xit "updates the main_image to be empty if given an empty cover_image" do
      ...
      xit "updates the main_image to be empty if given a different cover_image" do
        image = Faker::Avatar.image
        article.update(main_image: image)
        expect(article.main_image).to eq(image)
      ...
      xit "assigns the article to a new series belonging to the user" do
        expect do
          put_article(
            title: Faker::Book.title,
            body_markdown: "Yo ho ho",
      ...
      xit "assigns the article to an existing series belonging to the user" do
      ...
      xit "does not remove the article from a series" do
        collection = create(:collection, user: user)
        body_markdown = "Yo ho ho"
      ...
      xit "removes the article from a series if asked explicitly" do
        body_markdown = "Yo ho ho"
      ...
      xit "assigns the article to a series belonging to the article's owner, not the admin" do
        user.add_role(:super_admin)
        article = create(:article, user: create(:user))
      ...
      xit "does not update the editing time when updated before publication" do
      ...
      xit "updates the editing time when updated after publication" do
      ...
      xit "does not update the editing time before publication if changed by an admin" do
        article.update_columns(edited_at: nil)
        expect(article.published).to be(false)
        user.add_role(:super_admin)
        article = create(:article, user: create(:user))
      ...
      xit "does not update the editing time after publication if changed by an admin" do
        user.add_role(:super_admin)
      ...
      xit "updates the editing time when updated after publication if the owner is an admin" do
        user.add_role(:super_admin)
      ...
      xit "assigns the article to the organization" do
