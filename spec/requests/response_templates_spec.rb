...
RSpec.describe "ResponseTemplate", type: :request do
  ...
  describe "GET /response_templates #index" do
    it "returns not found if no user is logged in" do
      expect do
        get "/response_templates", headers: { HTTP_ACCEPT: "application/json" }
    ...
    context "when signed in as a regular user" do
      ...
      it "raises an error if trying to view moderator response templates" do
        create(:response_template, user: nil, type_of: "mod_comment")
        expect do
          get "/response_templates?type_of=mod_comment&personal_included=true", headers: { HTTP_ACCEPT: "application/json" }
        end.to raise_error Pundit::NotAuthorizedError
      end

      it "raises an error if trying to view admin response templates" do
        create(:response_template, user: nil, type_of: "email_reply")
        expect do
          get "/response_templates?type_of=email_reply", headers: { HTTP_ACCEPT: "application/json" }
        end.to raise_error Pundit::NotAuthorizedError
      end
    end
    ...
    context "when signed in as a mod user" do
      ...
      let(:url) { "/response_templates?type_of=mod_comment&personal_included=true" }
      ...
      it "raises unauthorized error if trying to view admin response templates" do
        create_list(:response_template, 2, user: nil, type_of: "email_reply")
        expect do
          get "/response_templates?type_of=email_reply", headers: { HTTP_ACCEPT: "application/json" }
        end.to raise_error Pundit::NotAuthorizedError
      end
