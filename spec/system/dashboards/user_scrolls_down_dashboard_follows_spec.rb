...
RSpec.describe "Infinite scroll on dashboard", type: :system, js: true do
  ...
  context "when /dashboard/following_tags is visited" do
    ...
    it "updates two tag point values" do
      ...
      within(last_divs[0]) { fill_in "follow_points", with: 5.0 }
      within(last_divs[1]) { fill_in "follow_points", with: 10.0 }

      click_button "commit"

      first_divs = page.all('div[id^="follows"]').first(2)
      within(first_divs[0]) { expect(page).to have_field("follow_points", with: 10.0) }
      within(first_divs[1]) { expect(page).to have_field("follow_points", with: 5.0) }
    end
  end
