...
RSpec.describe "Authenticating with Twitch" do
  let(:sign_in_link) { "Continue with Twitch" }

  before do
    omniauth_mock_twitch_payload
  ...
  context "when a user is new" do
    ...
    context "when using valid credentials but witholding email address" do
      before do
        OmniAuth.config.mock_auth[:twitch][:info].delete(:email)
        OmniAuth.config.mock_auth[:twitch][:extra][:raw_info].delete(:email)
      end
    ...
    context "when trying to register with an already existing username" do
      it "creates a new user with a temporary username" do
        # see Authentication::Providers::twitch#new_user_data
        username = OmniAuth.config.mock_auth[:twitch].info.nickname.downcase.sub(" ", "_")
    ...
    context "when using invalid credentials" do
      let(:params) do
        '{"callback_url"=>"http://localhost:3000/users/auth/twitch/callback", "state"=>"navbar_basic"}'
      end

      before do
        omniauth_setup_invalid_credentials(:twitch)
      ...
      it "notifies Datadog about a callback error" do
        ...
        args = omniauth_failure_args(error, "twitch", params)
    ...
    context "when a validation failure occurrs" do
      before do
        ...
        OmniAuth.config.mock_auth[:twitch].info.nickname = "X" * 101
      end
  ...
  context "when a user already exists" do
    ...
    let(:user) { create(:user, :with_identity, identities: [:twitch]) }

    before do
      auth_payload.info.nickname = user.name
    ...
    context "when already signed in" do
      it "redirects to the dashboard" do
        sign_in user
        visit user_twitch_omniauth_authorize_path
