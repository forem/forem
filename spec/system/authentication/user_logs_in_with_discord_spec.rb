...
RSpec.describe "Authenticating with Discord" do
  let(:sign_in_link) { "Continue with Discord" }

  before do
    omniauth_mock_discord_payload
  ...
  context "when a user is new" do
    ...
    context "when using valid credentials but witholding email address" do
      before do
        OmniAuth.config.mock_auth[:discord][:info].delete(:email)
        OmniAuth.config.mock_auth[:discord][:extra][:raw_info].delete(:email)
      end
    ...
    context "when trying to register with an already existing username" do
      it "creates a new user with a temporary username" do
        # see Authentication::Providers::Discord#new_user_data
        username = OmniAuth.config.mock_auth[:discord].info.name.downcase.sub(" ", "_")
    ...
    context "when using invalid credentials" do
      let(:params) do
        '{"callback_url"=>"http://localhost:3000/users/auth/discord/callback", "state"=>"navbar_basic"}'
      end

      before do
        omniauth_setup_invalid_credentials(:discord)
      ...
      it "notifies Datadog about a callback error" do
        ...
        args = omniauth_failure_args(error, "discord", params)
    ...
    context "when a validation failure occurrs" do
      before do
        ...
        OmniAuth.config.mock_auth[:discord].info.name = "X" * 101
      end
  ...
  context "when a user already exists" do
    let!(:auth_payload) { OmniAuth.config.mock_auth[:discord] }
    let(:user) { create(:user, :with_identity, identities: [:discord]) }
    ...
    context "when already signed in" do
      it "redirects to the dashboard" do
        sign_in user
        visit user_discord_omniauth_authorize_path
