...
class ExampleFormatter < RSpec::Core::Formatters::JsonFormatter
  ...
  def close(_notification)
    with_headers = { write_headers: true, headers: ["Example", "File", "Status", "Run Time", "Exception"] }
    CSV.open(output.path, "w", with_headers) do |csv|
      @output_hash[:examples].map do |ex|
        csv << [ex[:full_description], "#{ex[:file_path]}:#{ex[:line_number]}", ex[:status], ex[:run_time], ex[:exception]]
      end
    end
    puts "===============================writing csv"
    puts File.read("spec_log.csv")
  end
end

RSpec.configure do |c|
  c.add_formatter(ExampleFormatter, "spec_log.csv")
end
