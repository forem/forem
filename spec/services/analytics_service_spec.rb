...
RSpec.describe AnalyticsService, type: :service do
  ...
  describe "initialization" do
    xit "raises an error if start date is invalid" do
      expect(-> { described_class.new(user, start_date: "2000-") }).to raise_error(ArgumentError)
    end

    xit "raises an error if end date is invalid" do
      expect(-> { described_class.new(user, end_date: "2000-") }).to raise_error(ArgumentError)
    end

    xit "raises an error if an article does not belong to the user" do
  ...
  describe "#totals" do
    ...
    describe "comments stats" do
      ...
      xit "returns zero as total if there are no scored comments" do
    ...
    describe "reactions stats" do
      ...
      xit "returns the total number of reactions" do
        create(:reaction, reactable: article)
        expect(analytics_service.totals[:reactions][:total]).to eq(1)
      end

      xit "returns zero as total if there are no public category reactions" do
    ...
    describe "follows stats" do
      ...
      xit "returns the total number of follows" do
        create(:follow, followable: user)
        expected_stats = { total: 1 }
        expect(analytics_service.totals[:follows]).to eq(expected_stats)
      end

      xit "returns zero as total if there are no follows" do
        expected_stats = { total: 0 }
        expect(analytics_service.totals[:follows]).to eq(expected_stats)
      end
    end
    ...
    describe "page views stats" do
      ...
      xit "returns the total number of page views from page_views_count" do
        article.update_columns(page_views_count: 1)
        expect(analytics_service.totals[:page_views][:total]).to eq(1)
      end

      xit "returns zero as total if there are no page views" do
        expect(analytics_service.totals[:page_views][:total]).to eq(0)
      end

      xit "returns the average read time in seconds" do
      ...
      xit "returns zero as the average read time in seconds without views" do
        expect(analytics_service.totals[:page_views][:average_read_time_in_seconds]).to eq(0)
      end

      xit "returns the total read time in seconds" do
        article.update_columns(page_views_count: 1)
        create(:page_view, user: user, article: article, time_tracked_in_seconds: 15)
        create(:page_view, user: user, article: article, time_tracked_in_seconds: 45)
      ...
      xit "returns zero as the total read time in seconds with no page views" do
  ...
  describe "#grouped_by_day" do
    xit "returns stats grouped by day" do
    ...
    xit "returns stats for comments, reactions, follows and page views for a specific day" do
    ...
    xit "returns stats for an org" do
      stats = described_class.new(
    ...
    describe "page views stats for a specific day" do
      ...
      xit "returns the total number of page views from counts_for_number_of_views" do
      ...
      xit "works correctly if the page views contain nil in time_tracked_in_seconds" do
  ...
  describe "#referrers" do
    ...
    context "when working on domains" do
      ...
      xit "returns unique domains with a count" do
      ...
      xit "returns unique domains with a count when there is a date range" do
        url = Faker::Internet.url
        create(:page_view, user: user, article: article, referrer: url)
        create(:page_view, user: user, article: article, referrer: url)
        date = format_date(article.created_at)
      ...
      xit "returns nothing if there is no data" do
        expect(analytics_service.referrers[:domains]).to be_empty
      end

      xit "returns the domains ordered by count" do
      ...
      xit "returns 20 domains at most by default" do
      ...
      xit "returns the most visited domain if asked for only one result" do
        top_url = Faker::Internet.url
        create_list(:page_view, 3, user: user, article: article, referrer: top_url)
        other_url = Faker::Internet.url
        create_list(:page_view, 2, user: user, article: article, referrer: other_url)
