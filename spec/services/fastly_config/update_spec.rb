...
RSpec.describe FastlyConfig::Update, type: :service do
  ...
  before do
    ...
    allow(described_class).to receive(:get_active_version).and_return(fastly_version)
  ...
  describe "::run" do
    it "raises an error for incorrectly formatted configs" do
      expect { described_class.run(configs: "Not an Array") }.to raise_error(FastlyConfig::Errors::InvalidConfigsFormat)
    end

    it "raises an error for invalid configs" do
      expect { described_class.run(configs: ["Invalid config"]) }.to raise_error(FastlyConfig::Errors::InvalidConfig)
    end
