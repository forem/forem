...
RSpec.describe WebMentions::WebMentionHandler, type: :service do
  ...
  before { allow(described_class.new(canonical_url: canonical_url)).to receive(:call) }
  ...
  describe "webmentions handler" do
    context "when checking webmention support" do
      it "returns false on urls without webmention support" do
        ...
        expect(described_class.new(canonical_url: canonical_url).call).to be_falsey
      end
    end
    ...
    context "when sending webmentions" do
      it "sends a post request to the webmention url" do
        ...
        described_class.new(canonical_url: canonical_url, article_url: article_url,
                            webmention_url: webmention_url).__send__(:send_webmention)
