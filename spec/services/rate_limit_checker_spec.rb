...
RSpec.describe RateLimitChecker, type: :service do
  ...
  describe "#limit_by_action" do
    xit "returns false for invalid action" do
      expect(rate_limit_checker.limit_by_action("random-nothing")).to be(false)
    end

    xit "will limit action by ip_address if present" do
    ...
    xit "raises an error if no unique component is present for a cache key" do
      action = described_class::ACTION_LIMITERS.keys.first
    ...
    described_class::ACTION_LIMITERS.except(:published_article_creation).each do |action, _options|
      xit "returns true if #{action} limit has been reached" do
      ...
      xit "returns false if #{action} limit has NOT been reached" do
        allow(Rails.cache).to receive(:read).with(
          cache_key(action), raw: true
    ...
    context "when creating comments" do
      ...
      xit "returns true if too many comments at once" do
      ...
      xit "returns false if allowed comment" do
        expect(rate_limit_checker.limit_by_action("comment_creation")).to be(false)
      end
    end

    xit "returns true if too many published articles at once" do
    ...
    xit "returns true if a user has followed more than <daily_limit> accounts today" do
    ...
    xit "returns false if a user's following_users_count is less than <daily_limit>" do
      allow(user).
    ...
    xit "returns false if a user has followed less than <daily_limit> accounts today" do
      allow(rate_limit_checker).
        to receive(:user_today_follow_count).
    ...
    xit "returns false if published articles limit has not been reached" do
      expect(described_class.new(user).limit_by_action("published_article_creation")).to be(false)
    end

    xit "logs a rate limit hit to datadog" do
      allow(Rails.cache).
  ...
  describe "#check_limit!" do
    xit "returns nil if limit_by_action is false" do
    ...
    xit "raises an error if limit_by_action is true" do
  ...
  describe "#track_limit_by_action" do
    xit "increments cache for action with retry as expiration" do
  ...
  describe "#limit_by_email_recipient_address" do
    ...
    xit "returns true if too many emails are sent to the same recipient" do
    ...
    xit "returns false if we are below the message limit for this recipient" do
