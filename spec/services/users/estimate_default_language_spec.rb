...
RSpec.describe Users::EstimateDefaultLanguage, type: :service do
  ...
  xit "estimates sets preferred languages to [en] when no lang data" do
    user = create(:user)
    described_class.call(user)
    expect(user.decorate.preferred_languages_array).to eq(%w[en])
  end

  xit "estimates default language to be japan with jp email" do
    user = create(:user, email: "anna@example.jp")
    described_class.call(user)
    expect(user.estimated_default_language).to eq("ja")
  end

  xit "estimates default language based on identity data dump" do
    user = create(:user)
  ...
  xit "sets preferred_languages_array" do
  ...
  xit "sets correct language_settings for jp" do
  ...
  xit "sets correct language_settings for pt" do
    user = create(:user)
  ...
  xit "sets language settings when language is in twitter format (en-gb)" do
    user = create(:user)
