...
RSpec.describe EdgeCache::Service, type: :service do
  ...
  describe "#bust_fastly_cache" do
    context "when fastly is not configured" do
      ...
      let(:cache_service) { described_class.new(path) }

      it "does not bust a fastly cache" do
        allow(cache_service).to receive(:bust_fastly_cache)

        cache_service.call
        expect(cache_service.provider).to be(nil)
        expect(cache_service).not_to have_received(:bust_fastly_cache)
      end
    end
    ...
    context "when fastly is configured" do
      ...
      it "can bust a fastly cache" do
        ...
        cache_service.call
        expect(cache_service.provider).to eq("fastly")
        expect(cache_service).to have_received(:bust_fastly_cache)
      end
    end
  end
  ...
  describe "#bust_nginx_cache" do
    ...
    context "when openresty is not configured" do
      ...
      it "does not bust an nginx cache" do
        allow(cache_service).to receive(:bust_nginx_cache)

        cache_service.call
        expect(cache_service.provider).to eq(nil)
        expect(cache_service).not_to have_received(:bust_nginx_cache)
      end
    end
    ...
    context "when openresty is configured and available" do
      ...
      it "can bust an nginx cache" do
        ...
        cache_service.call
        expect(cache_service.provider).to eq("nginx")
        expect(cache_service).to have_received(:bust_nginx_cache)
      end
    end
  end
