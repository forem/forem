...
RSpec.describe BillboardEventRollup, type: :service do
  let(:ad1) { create(:billboard) }
  let(:ad2) { create(:billboard) }
  ...
  it "fails if new attributes would be lost" do
    attributes_considered = BillboardEventRollup::ATTRIBUTES_PRESERVED + BillboardEventRollup::ATTRIBUTES_DESTROYED
    expect(BillboardEvent.column_names.map(&:to_sym)).to contain_exactly(*attributes_considered)
  end
