...
RSpec.describe UserQueryExecutor do
  ...
  describe "#initialize" do
    it "sets up the executor with valid parameters" do
      ...
      expect(executor.timeout_ms).to eq(30000)
      expect(executor.limit).to be_nil
    end

    it "accepts custom timeout and limit" do
      executor = UserQueryExecutor.new(user_query, timeout_ms: 60000, limit: 100)
      expect(executor.timeout_ms).to eq(60000)
  ...
  describe "#execute" do
    ...
    it "handles query validation failures" do
      invalid_query = create(:user_query, query: "UPDATE users SET name = 'test'", created_by: user)
      executor = UserQueryExecutor.new(invalid_query)
  ...
  describe "error handling" do
    it "handles timeout errors" do
      ...
      expect(result).to be_empty
      expect(executor.error_messages).to include(match(/Query execution exceeded timeout/))
    end
  ...
  describe "execution environment setup" do
    it "sets statement timeout" do
      executor = UserQueryExecutor.new(user_query, timeout_ms: 45000)
  ...
  describe "query building" do
    ...
    it "respects maximum user limit" do
      executor = UserQueryExecutor.new(user_query, limit: 200000) # Over MAX_USER_LIMIT
      safe_query = executor.send(:build_safe_query)
