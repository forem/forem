...
RSpec.describe Github::OauthClient, type: :service, vcr: true do
  ...
  describe "initialization" do
    xit "raises ArgumentError if credentials are missing" do
      expect { described_class.new({}) }.to raise_error(ArgumentError)
    end

    xit "raises ArgumentError if access_token is empty" do
      expect { described_class.new(access_token: "") }.to raise_error(ArgumentError)
    end

    xit "raises ArgumentError if client_id or client_secret are empty" do
      expect { described_class.new(client_id: "value", client_secret: "") }.to raise_error(ArgumentError)
    end

    xit "succeeds if access_token is present" do
      expect { described_class.new(access_token: "value") }.not_to raise_error(ArgumentError)
    end

    xit "succeeds if both client_id and client_secret are present" do
      expect { described_class.new(client_id: "value", client_secret: "value") }.not_to raise_error(ArgumentError)
    end
  end

  describe ".issue" do
    xit "returns a an issue using the client_id/client_secret" do
    ...
    xit "returns a an issue using the access_token" do
