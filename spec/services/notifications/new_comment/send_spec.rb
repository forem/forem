...
RSpec.describe Notifications::NewComment::Send, type: :service do
  ...
  xit "creates users notifications" do
    expect do
  ...
  xit "creates the correct comment data for the notification" do
    described_class.call(child_comment)
  ...
  xit "creates notifications for the article author and the parent comment author" do
    described_class.call(child_comment)
  ...
  xit "creates notifications for all subscribed users" do
  ...
  xit "creates author comments notification" do
  ...
  xit "doesn't create a notification for top-level-only subscribed users" do
    create(:notification_subscription, user: top_level_subscriber, notifiable: article, config: "top_level_comments")
    described_class.call(child_comment)
    notified_user_ids = Notification.where(notifiable_type: "Comment", notifiable_id: child_comment.id).pluck(:user_id)
    expect(notified_user_ids.sort).to eq([user.id, user2.id].sort)
  ...
  xit "doesn't create a notification for the author of the article if they are not subscribed" do
    article.notification_subscriptions.delete_all
    described_class.call(comment)
    expect(Notification.count).to eq 0
  end

  xit "doesn't create a notification if the receive notification is false" do
  ...
  xit "creates an organization notification" do
  ...
  xit "sends Push Notifications using Pusher Beams when configured" do
