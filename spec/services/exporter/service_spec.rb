...
RSpec.describe Exporter::Service, type: :service do
  ...
  describe "EXPORTERS" do
    xit "is a list of supported exporters" do
  ...
  describe "#export" do
    xit "exports a zip file with files" do
    ...
    xit "passes configuration to an exporter" do
      service = valid_instance(article.user)
    ...
    xit "fetches the passed config" do
      service = valid_instance(article.user)
      config = double
    ...
    context "when emailing the user" do
      xit "delivers one email" do
        service = valid_instance(article.user)
        service.export(send_email: true)
        expect(ActionMailer::Base.deliveries.count).to eq(1)
      end

      xit "delivers an email with the export" do
        service = valid_instance(article.user)
    ...
    xit "sets the requested flag as false" do
      service = valid_instance(article.user)
      service.export
      expect(user.export_requested).to be(false)
    end

    xit "sets the exported at datetime as the current one" do
