...
RSpec.describe Payments::Customer, type: :service do
  ...
  describe ".get" do
    xit "retrieves an existing customer" do
      expect(described_class.get(customer.id)).to be_present
    end

    xit "raises Payments::CustomerNotFoundError if the customer does not exist" do
      expect { described_class.get("foobar") }.to raise_error(Payments::InvalidRequestError)
    end

    xit "increments stripe.errors if the customer does not exist" do
    ...
    xit "raises Payments::PaymentsError for any other known error" do
    ...
    xit "increments stripe.errors for any other known error" do
      allow(DatadogStatsClient).to receive(:increment)
      allow(Stripe::Customer).to receive(:retrieve).with("foobar").and_raise(Stripe::StripeError)
  ...
  describe ".create" do
    ...
    xit "raises an error if anything in the params is invalid" do
  ...
  describe ".create_source" do
    ...
    xit "increments stripe.errors if anything in the params is invalid" do
      error = Stripe::InvalidRequestError.new("message", :token)
      allow(Stripe::Customer).to receive(:create_source).and_raise(error)
      allow(DatadogStatsClient).to receive(:increment)
  ...
  describe ".get_source" do
    xit "gets an existing source" do
    ...
    xit "raises Payments::InvalidRequestError if the source does not exist" do
  ...
  describe ".detach_source" do
    xit "removes an existing source" do
      source = Stripe::Customer.create_source(customer.id, source: "token")
  ...
  describe ".charge" do
    ...
    xit "raises a card error if the card has any troubles" do
    ...
    xit "increments stripe.errors if the card has any troubles" do
      StripeMock.prepare_card_error(:expired_card)
      allow(DatadogStatsClient).to receive(:increment)
