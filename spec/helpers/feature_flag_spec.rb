...
describe FeatureFlag, type: :helper do
  describe ".strict_enabled?" do
    it "calls Flipper's enabled? method" do
      ...
      described_class.strict_enabled?("foo")
  ...
  describe ".enabled?" do
    ...
    context "when flag doesn't exist" do
      it "returns true" do
        expect(described_class.enabled?("missing_flag")).to be_truthy # rubocop:disable Rspec/PredicateMatcher
      end
    end
    ...
    context "when flag exists and is set to off" do
      ...
      it "returns false" do
        expect(described_class.enabled?("flag")).to be_falsy # rubocop:disable Rspec/PredicateMatcher
      end

      it "returns true when flag is on for user" do
        Flipper.enable_actor("flag", user)
        expect(described_class.enabled?("flag", user)).to be_truthy # rubocop:disable Rspec/PredicateMatcher
      end
    end
    ...
    context "when flag exists and is set to on" do
      ...
      it "returns true" do
        expect(described_class.enabled?("flag")).to be_truthy # rubocop:disable Rspec/PredicateMatcher
      end
