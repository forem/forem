...
RSpec.describe "layouts/_signup_modal.html.erb", type: :view do
  ...
  it "renders the tagline if it is set" do
    allow(Settings::General).to receive(:tagline).and_return(tagline_text)
    render
    expect(rendered).to have_text(Settings::General.tagline)
  end

  it "does not render the tagline if it is not set" do
    allow(Settings::General).to receive(:tagline).and_return(nil)
    render
