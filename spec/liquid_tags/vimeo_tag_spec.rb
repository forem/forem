...
RSpec.describe VimeoTag, type: :liquid_tag do
  ...
  xit "accepts vimeo video id" do
    assert_parses id, id
  end

  xit "accepts vimeo video id with wonky whitespace" do
    assert_parses id, " #{id}  \t"
  end

  xit "accepts a vimeo video url" do
    assert_parses id, "https://vimeo.com/#{id}"
    assert_parses id, "vimeo.com/#{id}"
  end

  xit "accepts a vimeo player url" do
  ...
  xit "accepts urls that were over-eagerly turned into links by markdown" do
