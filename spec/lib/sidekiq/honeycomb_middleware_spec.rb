...
RSpec.describe Sidekiq::HoneycombMiddleware do
  ...
  it "sends an event with expected keys" do
    ...
    collected_data = Honeycomb.libhoney.send("client").events.map(&:data).detect { |h| h["sidekiq.args"] == expected_hash["sidekiq.args"] }
  ...
  context "with an error" do
    ...
    it "sends an event with expected keys" do
      ...
      collected_data = Honeycomb.libhoney.send("client").events.map(&:data).detect { |h| h["sidekiq.args"] == error_hash["sidekiq.args"] }
