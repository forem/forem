...
RSpec.describe ReverseMarkdown::Converters::CustomPre, type: :lib do
  ...
  describe "#convert" do
    xit "detects languages from language-prefixed classnames" do
    ...
    xit "detects languages from highlight-prefixed classnames on the parent element" do
      node = Nokogiri::HTML('<div class="highlight-ruby"><pre></pre></div>').search("pre")[0]
      result = create_custom_pre.convert(node)
      expect(result.split[0]).to eq("```ruby")
    end

    xit "detects languages from confluence-style classnames" do
