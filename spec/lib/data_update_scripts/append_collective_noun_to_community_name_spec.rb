...
describe DataUpdateScripts::AppendCollectiveNounToCommunityName do
  context "when collective_noun_disabled is false" do
    it "appends the collective_noun to the community_name" do
      allow(Settings::General).to receive(:collective_noun).and_return("Club")
      described_class.new.run
      expect(Settings::General.collective_noun_disabled).to eq(false)
      expect(Settings::General.community_name).to eq("DEV(local) Club")
    end
  end

  context "when collective_noun_disabled is true" do
    it "does not append the collective_noun to the community_name" do
      allow(Settings::General).to receive(:collective_noun_disabled).and_return(true)
      described_class.new.run
      expect(Settings::General.community_name).to eq("DEV(local)")
    end
  end
end
