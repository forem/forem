...
RSpec.describe CommentDecorator, type: :decorator do
  ...
  describe "#low_quality" do
    ...
    xit "returns true if the comment is low quality" do
      comment.score = threshold - 1
      expect(comment.decorate.low_quality).to be(true)
    end

    xit "returns false if the comment score is on threshold quality" do
      comment.score = threshold
      expect(comment.decorate.low_quality).to be(false)
    end

    xit "returns false if the comment is a good quality" do
      comment.score = threshold + 1
      expect(comment.decorate.low_quality).to be(false)
    end
  end
  ...
  describe "#published_timestamp" do
    xit "returns empty string if the comment is new" do
      expect(Comment.new.decorate.published_timestamp).to eq("")
    end

    xit "returns the timestamp of the creation date" do
  ...
  describe "#published_at_int" do
    ...
    xit "returns the creation date as an integer" do
