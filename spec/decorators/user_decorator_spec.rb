...
RSpec.describe UserDecorator, type: :decorator do
  ...
  describe "#cached_followed_tags" do
    ...
    xit "returns empty if no tags followed" do
      expect(saved_user.decorate.cached_followed_tags.size).to eq(0)
    end

    xit "returns array of tags if user follows them" do
  ...
  describe "#darker_color" do
    xit "returns a darker version of the assigned color if colors are blank" do
      saved_user.assign_attributes(bg_color_hex: "", text_color_hex: "")
      expect(saved_user.decorate.darker_color).to be_present
    end

    xit "returns a darker version of the color if bg_color_hex is present" do
    ...
    xit "returns an adjusted darker version of the color" do
      saved_user.assign_attributes(bg_color_hex: "#dddddd", text_color_hex: "#ffffff")
      expect(saved_user.decorate.darker_color(0.3)).to eq("#424242")
    end

    xit "returns an adjusted lighter version of the color if adjustment is over 1.0" do
      saved_user.assign_attributes(bg_color_hex: "#dddddd", text_color_hex: "#ffffff")
  ...
  describe "#enriched_colors" do
    xit "returns assigned colors if bg_color_hex is blank" do
    ...
    xit "returns assigned colors if text_color_hex is blank" do
      saved_user.assign_attributes(text_color_hex: "")
      expect(saved_user.decorate.enriched_colors[:bg]).to be_present
      expect(saved_user.decorate.enriched_colors[:text]).to be_present
    end

    xit "returns bg_color_hex and assigned text_color_hex if text_color_hex is blank" do
      saved_user.assign_attributes(bg_color_hex: "#dddddd", text_color_hex: "")
      expect(saved_user.decorate.enriched_colors[:bg]).to be_present
      expect(saved_user.decorate.enriched_colors[:text]).to be_present
    end

    xit "returns text_color_hex and assigned bg_color_hex if bg_color_hex is blank" do
      saved_user.assign_attributes(bg_color_hex: "", text_color_hex: "#ffffff")
      expect(saved_user.decorate.enriched_colors[:bg]).to be_present
      expect(saved_user.decorate.enriched_colors[:text]).to be_present
    end

    xit "returns bg_color_hex and text_color_hex if both are present" do
  ...
  describe "#config_body_class" do
    ...
    xit "creates proper body class with minimal light theme" do
    ...
    xit "works with static navbar" do
    ...
    context "when user with roles" do
      ...
      xit "creates proper body class with trusted user" do
  ...
  describe "#fully_banished?" do
    xit "returns not fully banished if in good standing" do
      expect(user.decorate.fully_banished?).to eq(false)
    end

    xit "returns fully banished if user has been banished" do
  ...
  describe "#stackbit_integration?" do
    ...
    xit "returns true if the user has access tokens" do
