...
RSpec.describe ArticleDecorator, type: :decorator do
  ...
  describe "#processed_canonical_url" do
    it "strips canonical_url" do
      ...
      expect(article.decorate_.processed_canonical_url). to eq("http://google.com")
    end

    it "returns the article url without a canonical_url" do
      article.canonical_url = ""
      expected_url = "https://#{ApplicationConfig['APP_DOMAIN']}#{article.path}"
      expect(article.decorate_.processed_canonical_url).to eq(expected_url)
    end
  end
  ...
  describe "#comments_to_show_count" do
    it "returns 25 if does not have a discuss tag" do
      ...
      expect(article.decorate_.comments_to_show_count).to eq(25)
    end

    it "returns 75 if it does have a discuss tag" do
      article.cached_tag_list = "discuss, python"
      expect(article.decorate_.comments_to_show_count).to eq(75)
    end
  end
  ...
  describe "#cached_tag_list_array" do
    it "returns no tags if the cached tag list is empty" do
      article.cached_tag_list = ""
      expect(article.decorate_.cached_tag_list_array).to be_empty
    end

    it "returns cached tag list as an array" do
      article.cached_tag_list = "discuss, python"
      expect(article.decorate_.cached_tag_list_array).to eq(%w[discuss python])
    end
  end
  ...
  describe "#url" do
    it "returns the article url" do
      expected_url = "https://#{ApplicationConfig['APP_DOMAIN']}#{article.path}"
      expect(article.decorate_.url).to eq(expected_url)
    end
  end
  ...
  describe "#title_length_classification" do
    it "returns article title length classifications" do
      article.title = "0" * 106
      expect(article.decorate_.title_length_classification).to eq("longest")
      article.title = "0" * 81
      expect(article.decorate_.title_length_classification).to eq("longer")
      article.title = "0" * 61
      expect(article.decorate_.title_length_classification).to eq("long")
      article.title = "0" * 23
      expect(article.decorate_.title_length_classification).to eq("medium")
      article.title = "0" * 20
      expect(article.decorate_.title_length_classification).to eq("short")
    end
  end
  ...
  describe "internal_utm_params" do
    it "returns utm params for a boosted article" do
      ...
      expect(article.decorate_.internal_utm_params).to eq(expected_result)
    end
    ...
    it "returns utm params for an article in a different place" do
      ...
      expect(article.decorate_.internal_utm_params("homepage")).to eq(expected_result)
    end
  end

  describe "#published_at_int" do
    it "returns the publication date as an integer" do
      expect(article.decorate_.published_at_int).to eq(article.published_at.to_i)
    end
  end
