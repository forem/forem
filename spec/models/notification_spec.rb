  ...
  describe "#cleanup_old_notifications" do
    context "when ENABLE_USER_NOTIFICATION_CLEANUP is not set" do
      ...
      it "does not enqueue a cleanup job" do
        user.update!(created_at: 100.days.ago)
        allow_any_instance_of(described_class).to receive(:rand).with(15).and_return(0)
    ...
    context "when ENABLE_USER_NOTIFICATION_CLEANUP is true" do
      ...
      it "enqueues a cleanup job 1-in-15 times if user is older than 90 days" do
        user.update!(created_at: 100.days.ago)
        allow_any_instance_of(described_class).to receive(:rand).with(15).and_return(0)
      ...
      it "does not enqueue a cleanup job if user is newer than 90 days" do
        user.update!(created_at: 80.days.ago)
        allow_any_instance_of(described_class).to receive(:rand).with(15).and_return(0)
      ...
      it "does not enqueue a cleanup job 14/15th of the time" do
        user.update!(created_at: 100.days.ago)
        allow_any_instance_of(described_class).to receive(:rand).with(15).and_return(1)
