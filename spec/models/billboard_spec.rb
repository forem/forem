...
RSpec.describe Billboard do
  ...
  describe "after_save callbacks" do
    ...
    it "generates a name when one does not exist" do
      ...
      expect(billboard.name).to eq("Display Ad #{billboard.id}")
      expect(billboard_with_name.name).to eq("Test")
    end
  end

  describe ".search_ads" do
    let!(:ad) { create(:billboard, name: "This is an Ad", body_markdown: "Ad Body", placement_area: "post_comments") }
  ...
  describe "seldom_seen scope" do
    ...
    let!(:priority_ad) { create(:billboard, priority: true, impressions_count: low_impression_count + 1) }
