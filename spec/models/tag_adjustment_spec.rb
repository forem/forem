...
RSpec.describe TagAdjustment, type: :model do
  ...
  describe "privileges" do
    xit "allows tag mods to create for their tags" do
    ...
    xit "does not allow tag mods to create for other tags" do
    ...
    xit "allows admins to create for any tags" do
      tag_adjustment = build(:tag_adjustment, user_id: admin_user.id, article_id: article.id, tag_id: tag.id)
      expect(tag_adjustment).to be_valid
    end

    xit "does not allow normal users to create for any tags" do
  ...
  describe "allowed attribute states" do
    xit "allows addition adjustment_types" do
      tag_adjustment = build(:tag_adjustment, user_id: mod_user.id, article_id: article.id, tag_id: tag.id)
      expect(tag_adjustment).to be_valid
    end

    xit "allows removal adjustment_types" do
  ...
  describe "validates article tag_list" do
    xit "does not allow addition on articles with 4 tags" do
