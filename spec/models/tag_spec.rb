...
RSpec.describe Tag, type: :model do
  ...
  describe ".cached_followed_tags_for" do
    ...
    it "returns empty if no tags followed" do
      expect(described_class.cached_followed_tags_for(follower: saved_user).size).to eq(0)
    end
    ...
    it "returns array of tags if user follows them" do
      ...
      expect(described_class.cached_followed_tags_for(follower: saved_user).size).to eq(3)
    end

    it "returns tag object with name" do
      saved_user.follow(tag1)
      expect(described_class.cached_followed_tags_for(follower: saved_user).first.name).to eq(tag1.name)
    end

    it "returns follow points for tag" do
      saved_user.follow(tag1)
      expect(described_class.cached_followed_tags_for(follower: saved_user).first.points).to eq(1.0)
    end
    ...
    it "returns adjusted points for tag" do
      ...
      expect(described_class.cached_followed_tags_for(follower: saved_user).first.points).to eq(0.1)
    end
  end
