...
RSpec.describe Tag, type: :model do
  ...
  describe "validations" do
    describe "bg_color_hex" do
      xit "passes validations if bg_color_hex is valid" do
        tag.bg_color_hex = "#000000"
        expect(tag).to be_valid
      end

      xit "fails validation if bg_color_hex is invalid" do
    ...
    describe "text_color_hex" do
      xit "passes validations if text_color_hex is valid" do
        tag.text_color_hex = "#000000"
        expect(tag).to be_valid
      end

      xit "fails validation if text_color_hex is invalid" do
    ...
    describe "name" do
      xit "passes validations if name is alphanumeric" do
        tag.name = "foobar123"
        expect(tag).to be_valid
      end

      xit "fails validations if name is empty" do
        tag.name = ""
        expect(tag).not_to be_valid
      end

      xit "fails validations if name is nil" do
        tag.name = nil
        expect(tag).not_to be_valid
      end
    end
    ...
    describe "alias_for" do
      xit "passes validation if the alias refers to an existing tag" do
        tag = create(:tag)
        tag.alias_for = tag.name
        expect(tag).to be_valid
      end

      xit "fails validation if the alias does not refer to an existing tag" do
        tag.alias_for = "hello"
        expect(tag).not_to be_valid
      end
    end
  ...
  describe "#after_commit" do
    xit "on update enqueues job to index tag to elasticsearch" do
      tag.save
    ...
    xit "on destroy enqueues job to delete tag from elasticsearch" do
      tag.save
