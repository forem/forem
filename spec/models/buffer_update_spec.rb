...
RSpec.describe BufferUpdate, type: :model do
  ...
  it "does not allow duplicate updates if the first one was a little while ago" do
    b1 = described_class.buff!(article.id, "twitter_buffer_text", "CODE")
    b1.update_column(:created_at, 5.minutes.ago)
    described_class.buff!(article.id, "twitter_buffer_text", "CODE")
    expect(described_class.all.size).to eq(2)
    described_class.buff!(article.id, "twitter_buffer_text", "CODE")
    expect(described_class.all.size).to eq(2)
    described_class.buff!(article.id, "twitter_buffer_text yoyo", "CODE")
  ...
  it "does not allow more than 3 suggestions" do
    Array.new(3) do |i|
      described_class.buff!(article.id, "twitter_buffer_text_#{i}", "CODE")
    end
    invalid_buffer = described_class.buff!(article.id, "twitter_buffer_text_4", "CODE")
    expect(described_class.all.size).to eq(3)
