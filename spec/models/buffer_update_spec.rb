...
RSpec.describe BufferUpdate, type: :model do
  ...
  xit "creates update" do
    described_class.buff!(article.id, "twitter_buffer_text", "CODE", "twitter")
    expect(described_class.all.size).to eq(1)
  end

  xit "does not allow duplicate updates" do
    described_class.buff!(article.id, "twitter_buffer_text", "CODE", "twitter")
    described_class.buff!(article.id, "twitter_buffer_text", "CODE", "twitter")
    expect(described_class.all.size).to eq(1)
  end

  xit "does not allow duplicate updates if the first one was a little while ago" do
  ...
  xit "allows same text across different social platforms" do
    described_class.buff!(article.id, "twitter_buffer_text", "CODE", "facebook")
    described_class.buff!(article.id, "twitter_buffer_text", "CODE", "twitter")
    expect(described_class.all.size).to eq(2)
  end

  xit "allows same text across different tags" do
  ...
  xit "allows same text across different articles with the same tag" do
    described_class.buff!(article.id, "twitter_buffer_text", "CODE", "twitter", 1)
    described_class.buff!(create(:article).id, "twitter_buffer_text", "CODE", "twitter", 1)
    expect(described_class.all.size).to eq(2)
  end

  xit "does not allow more than 3 suggestions" do
    Array.new(3) do |i|
