describe('Publish or save a post', () => {
  describe('v1 Editor', () => {
    ...
    it('Publishes a post without errors', () => {
      cy.findByRole('form', { name: /^Edit post$/i }).within(() => {
        ...
        cy.get('@postContent').type(validPublishedArticleContent);
    ...
    it('Saves a draft without errors', () => {
      cy.findByRole('form', { name: /^Edit post$/i }).within(() => {
        cy.findByLabelText('Post Content').as('postContent');
        cy.get('@postContent').clear();
        cy.get('@postContent').type(validDraftArticleContent);
        cy.findByRole('button', { name: 'Save changes' }).click();
      });
    ...
    it('Shows an error message when markdown is incorrect', () => {
      cy.findByRole('form', { name: /^Edit post$/i }).within(() => {
        cy.findByLabelText('Post Content').as('postContent');
        cy.get('@postContent').clear();
        cy.get('@postContent').type(invalidPublishedArticleContent, {
  ...
  describe('v2 Editor', () => {
    ...
    it('Publishes a post without errors', () => {
      cy.findByRole('form', { name: /^Edit post$/i }).within(() => {
        cy.findByLabelText('Post Title').as('postTitle');
        cy.get('@postTitle').clear();
        cy.get('@postTitle').type('Test title');
        cy.findByLabelText('Post Content').as('postContent');
        cy.get('@postContent').clear();
    ...
    it('Maintains draft status when editing a draft fails', () => {
      ...
      cy.findByLabelText('Post Content').as('postContent');
      cy.get('@postContent').clear();
      cy.get('@postContent').type('something else');
