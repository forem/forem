  ...
  describe('As an admin', () => {
    beforeEach(() => {
      ...
      cy.get('@user').then((user) => {
        cy.loginAndVisit(user, '/admin/member_management/users/2');
      });
    });
    ...
    it(`navigates to the "Notes" tab`, () => {
      ...
      cy.url().should('contain', '/admin/member_management/users/2?tab=notes');
    });

    it(`navigates to the "Emails" tab`, () => {
      cy.findByRole('link', { name: /Emails/i }).should('exist');
      cy.findByRole('link', { name: /Emails/i }).click();
      cy.url().should('contain', '/admin/member_management/users/2?tab=emails');
    });
    ...
    it(`navigates to the "Reports" tab`, () => {
      ...
      cy.url().should(
        'contain',
        '/admin/member_management/users/2?tab=reports',
      );
    });
    ...
    it(`navigates to the "Flags" tab`, () => {
      ...
      cy.url().should('contain', '/admin/member_management/users/2?tab=flags');
    });
  });
});
