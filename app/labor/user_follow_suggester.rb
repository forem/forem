  ...
  def suggestions
    ...
    if user.decorate.cached_followed_tag_names.any?
      ...
      group_3 = User.where(id: user_ids).
        order("articles_count DESC").limit(35).to_a
      group_4 = User.order("reputation_modifier DESC").offset(rand(0..offset_number)).limit(15).to_a
      group_5 = User.order("comments_count DESC").offset(rand(0..offset_number)).limit(15).to_a
