...
function setCollectionFunctionality() {
  if (document.getElementById('collection-link-inbetween')) {
    ...
    for (var i = 0; i < inbetweenLinks.length; i += 1) {
      inbetweenLinks[i].onclick = function onClickInbetweenLinks(e) {
        e.preventDefault();
...
function reactToArticle(articleId, reaction) {
  ...
  getCsrfToken()
    ...
    .then(function getCsrfTokenCallback(response) {
    ...
    .catch(function getCsrfTokenCatch(error) {
      toggleReaction();
      document.getElementById('reaction-butt-' + reaction).disabled = false;
    });
}
...
function initializeArticleReactions() {
  ...
  setTimeout(function reactionsTimeout() {
    if (document.getElementById('article-body')) {
      ...
      if (document.getElementById('article-reaction-actions')) {
        ...
        ajaxReq.onreadystatechange = function updateReactionsOnReady() {
          if (ajaxReq.readyState === XMLHttpRequest.DONE) {
            var json = JSON.parse(ajaxReq.response);
            json.article_reaction_counts.forEach(function initSetReactionCount(
              reaction,
            ) {
              setReactionCount(reaction.category, reaction.count);
            });
            json.reactions.forEach(function initShowUserReaction(reaction) {
              if (
    ...
    for (var i = 0; i < reactionButts.length; i += 1) {
      reactionButts[i].onclick = function initReactToArticle(e) {
    ...
    if (document.getElementById('jump-to-comments')) {
      document.getElementById(
        'jump-to-comments',
      ).onclick = function jumpToComments(e) {
        e.preventDefault();
