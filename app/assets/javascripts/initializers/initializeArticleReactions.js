        ...
        ajaxReq.onreadystatechange = function() {
          if (ajaxReq.readyState == XMLHttpRequest.DONE) {
            ...
            json.article_reaction_counts.forEach(function(reaction) {
              if (reaction.count > 0) {
                document
                  .getElementById('reaction-butt-' + reaction.category)
                  .classList.add('activated');
                document.getElementById(
                  'reaction-number-' + reaction.category,
                ).innerHTML = reaction.count;
              }
            });
...
function reactToArticle(articleId, reaction) {
  ...
  if (userStatus == 'logged-out') {
    ...
    document
      .getElementById('reaction-butt-' + reaction)
      .classList.add('user-activated');
    document
      .getElementById('reaction-butt-' + reaction)
      .classList.add('user-animated');
  }
  ...
  function successCb(response) {
    ...
    if (response.result == 'create') {
      ...
      document
        .getElementById('reaction-butt-' + reaction)
        .classList.remove('user-activated');
      if (num == 1) {
        document
          .getElementById('reaction-butt-' + reaction)
