module Articles
  class Attributes
    ATTRIBUTES = %i[archived body_markdown canonical_url description compellingness_score labels
                    edited_at main_image organization_id user_id published clickbait_score
                    title video_thumbnail_url published_at co_author_ids_list].freeze

    attr_reader :attributes, :article_user, :organization

    def initialize(attributes, article_user, organization: nil)
      @attributes = attributes
      @article_user = article_user
      @organization = organization
    end

    def for_update(update_edited_at: false)
      hash = attributes.slice(*ATTRIBUTES)
      # don't reset the collection when no series was passed
      hash[:collection] = collection if attributes.key?(:series)
      # Only include tag_list if tags are explicitly provided in the request.
      # This prevents losing existing tags when updating an article without specifying tags.
      hash[:tag_list] = tag_list if attributes.key?(:tag_list) || attributes.key?(:tags)
      hash[:edited_at] = Time.current if update_edited_at
      hash[:published_at] = hash[:published_at].to_datetime if hash[:published_at]
      hash
    end

    private

    def collection
      return unless attributes[:series].present?

      # Use organization from attributes if being changed, otherwise use the passed organization
      org = if attributes[:organization_id].present?
              Organization.find_by(id: attributes[:organization_id])
            else
              organization
            end
      Collection.find_series(attributes[:series], article_user, organization: org)
    end

    def tag_list
      if attributes[:tag_list]
        attributes[:tag_list]
      elsif attributes[:tags]
        attributes[:tags].join(", ")
      end
    end
  end
end
