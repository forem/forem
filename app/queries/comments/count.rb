...
#   - super low quality: scored below HIDE_THRESHOLD, or below LOW_QUALITY_THRESHOLD and containing a link
#     (mirrors CommentDecorator#super_low_quality), or
  ...
  class Count
    ...
    def call
      if recalculate || article.displayed_comments_count.nil?
        ...
        #   (a) it is super low quality (see CommentDecorator#super_low_quality), or
        ...
        count_sql = "SELECT COUNT(id) FROM comments c1 " \
                    "WHERE (score < ? OR (score < ? AND processed_html LIKE '%<a%') OR deleted = TRUE) " \
                    "AND commentable_id = ? " \
        ...
        san_count_sql = Comment.sanitize_sql([count_sql, Comment::HIDE_THRESHOLD, Comment::LOW_QUALITY_THRESHOLD,
                                                    @article.id, "Article"])
