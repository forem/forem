...
function blockChat(activeChannelId) {
  ...
  window.getCsrfToken().then(window.sendFetch('block-chat', formData));
}

/**
 * TODO: Instead of calling this function in render, use jsx (<SetupButton />).
 */
function SetupButton({ modalId = '', otherModalId = '', btnName = '' }) {
  return (
    <button
      type="button"
      onClick={() => {
...
const UserDetails = props => {
  const { user, activeChannelId: channelId, activeChannel } = props;
