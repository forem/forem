...
export class ReadingList extends Component {
  ...
  render() {
    const { readingListItems, availableTags, selectedTags, itemsLoaded, query, statusView, archiving } = this.state;
    let allItems = readingListItems.map(item => (
      <div className='readinglist-item-wrapper'>
        <a className='readinglist-item' href={item.searchable_reactable_path}>
          <div className='readinglist-item-title'>{item.searchable_reactable_title}</div>
        ...
        <button className='readinglist-archive-butt' onClick={e => this.archive(e, item)} type='button'>{statusView === 'valid' ? 'archive' : 'unarchive'}</button>
      </div>
    ));
    if (readingListItems.length === 0 && itemsLoaded) {
      allItems = (
        <div className='readinglist-empty'>
          ...
          <h3>
            Hit the <span>SAVE</span> or <span>Bookmark 🔖</span> to start your Collection
          </h3>
        </div>
      )
    }
    const allTags = availableTags.map(tag => (
      <a className={`readinglist-tag ${selectedTags.indexOf(tag) > -1 ? 'selected' : ''}`} href={`/t/${tag}`} data-no-instant onClick={e => this.toggleTag(e, tag)}>#{tag}</a>
    ))
    const snackBar = archiving ? <div className='snackbar'>{statusView === 'valid' ? 'Archiving' : 'Unarchiving' } (async)</div>: ''
    return (
      <div className='home readinglist-home'>
        <div className='side-bar'>
          <div className='widget readinglist-filters'>
            ...
            <div className='readinglist-tags'>
              {allTags}
            </div>
            <div className='readinglist-view-toggle'>
              <a href='/readinglist/archive' onClick={e => this.toggleStatusView(e)} data-no-instant>{statusView === 'valid' ? 'View Archive' : 'View Reading List'}</a>
            </div>
          </div>
        </div>
        <div className={`readinglist-results ${itemsLoaded ? 'readinglist-results--loaded' : ''}`}>
