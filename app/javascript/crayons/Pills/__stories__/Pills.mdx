...
Pills, expect just representing some date, should also have associated action (e.g. "remove", "edit", ...) but only one per pill.
...
like "you can't remove ABC role becuase you don't have appropraite privilages").
...
`noAction` prop which will apply approriate attributes to the button (`aria-disabled="true" readonly="true"`) - this will keep the Pill
accessible.
