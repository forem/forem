...
`<Link />` component is just an HTML's `<a>` element under the hood so it can use the same attributes, like `href` for setting path, `target` for defininig whether link should open in new window, etc.
...
Icon-only links should always use clear and obvious icon, ideally accompanied by a title.
