...
The only required prop is `description` which should be describing purpose of the Toggle.
It's there for accessibility reasons so it will only be "visible" to screen-readers.

Toggles in general should be used alongside actual `<label>s` providing all the necessary context and purpose for a given form element.
