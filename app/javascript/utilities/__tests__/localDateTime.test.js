...
describe('LocalDateTime Utilities', () => {
  ...
  describe('timestampToLocalDateTimeShort', () => {
    ...
    it('should format date with year for past year', () => {
      ...
      expect(result).toMatch(new RegExp(`Oct \\d+.*${pastYear}`));
    });
    ...
    it('should return empty string for invalid datetime strings', () => {
      // Exercises the try/catch in timestampToLocalDateTime
      const result = timestampToLocalDateTimeShort('not-a-valid-date');
      expect(result).toEqual('');
    });

    it('should return empty string for empty string timestamp', () => {
      const result = timestampToLocalDateTimeShort('');
      expect(result).toEqual('');
    });

    it('should fall back gracefully when navigator.language is an invalid locale', () => {
      Object.defineProperty(navigator, 'language', {
        writable: true,
        configurable: true,
        value: 'invalid-LOCALE-999',
      });

      const timestamp = '2024-10-15T10:00:00.000Z';
      // Should not throw — Intl.DateTimeFormat falls back to 'default' or catches
      expect(() => timestampToLocalDateTimeShort(timestamp)).not.toThrow();
    });
  });
  ...
  describe('localizeTimeElements', () => {
    ...
    it('should handle multiple time elements', () => {
      ...
      expect(time1.textContent).toMatch(/Oct \d+/);
      expect(time2.textContent).toMatch(/Oct \d+/);
    });
  });
});
