...
describe('formatAllTimeElements', () => {
  ...
  it('should preserve trailing whitespace from original text node when element has children', () => {
    const timeElement = document.createElement('time');
    timeElement.setAttribute('datetime', '2024-10-15T10:00:00.000Z');

    // Simulate the structure PublishDate.jsx produces: text node + child span
    timeElement.appendChild(document.createTextNode('Oct 15, 2024 '));
    const span = document.createElement('span');
    span.textContent = '(5 min ago)';
    timeElement.appendChild(span);
    ...
    // The text node should end with the preserved trailing space
    const textNode = Array.from(timeElement.childNodes).find(
      (n) => n.nodeType === Node.TEXT_NODE,
    );
    expect(textNode.textContent).toMatch(/\s+$/);
    // The child span should still be intact
    expect(timeElement.querySelector('span')).not.toBeNull();
  });

  it('should not re-process elements already marked with data-localized sentinel', () => {
    const timeElement = document.createElement('time');
    timeElement.setAttribute('datetime', '2024-10-15T10:00:00.000Z');
    timeElement.textContent = 'Already formatted';
    timeElement.setAttribute('data-localized', 'true');
    document.body.appendChild(timeElement);

    formatAllTimeElements();

    // Should not have called the formatter for a pre-processed element
    expect(timestampToLocalDateTimeShort).not.toHaveBeenCalled();
    // Text should be unchanged
    expect(timeElement.textContent).toBe('Already formatted');
  });

  it('should mark element with data-localized after formatting', () => {
    const timeElement = document.createElement('time');
    timeElement.setAttribute('datetime', '2024-10-15T10:00:00.000Z');
    timeElement.textContent = 'Oct 15, 2024';
    document.body.appendChild(timeElement);
    ...
    expect(timeElement.getAttribute('data-localized')).toBe('true');
  });
