...
if (Runtime != null && Runtime.currentMedium() !== 'ForemWebView') {
  // No need to load the namespaced functions because we're not in ForemWebView
  return;
}
...
window.ForemMobile = {
  ...
  getUserData () {
    ...
    if (userStatus === 'logged-in') {
      return document.getElementsByTagName('body')[0].getAttribute('data-user');
    }
  },
  getInstanceMetadata () {
    ...
    fetch('/users/devices', {
      ...
      .catch(() => {
        ...
        if (window.ForemMobile.retryDelayMs < 20000) {
          window.ForemMobile.retryDelayMs = window.ForemMobile.retryDelayMs * 2;
        }
