module Events
  class ManageBroadcastBillboardsWorker
    ...
    def perform
      ...
      # Look for upcoming events (start_time is in the future)
      # We want to explicitly ignore these so we don't accidentally teardown
      # pre-event published billboards.
      upcoming_events = Event.where("start_time > :start_cutoff", start_cutoff: start_cutoff).pluck(:id)
      ...
      # Step 2: Any billboard owned by a broadcast_config event that is NOT currently active AND NOT upcoming, should be unapproved.
      events_to_keep_alive = active_events + upcoming_events
      
      billboards_to_disable = Billboard.where.not(event_id: nil)
      billboards_to_disable = billboards_to_disable.where.not(event_id: events_to_keep_alive) if events_to_keep_alive.any?
