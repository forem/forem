module Metrics
  class RecordDailyUsageWorker
    ...
    def perform
      articles_min_15_score_past_24h = Article.published.where("score >= ? AND published_at > ?", 15, 1.day).size
      DataDogStatsClient.count("articles.min_15_score_past_24h", articles_min_15_score_past_24h, tags: { resource: "articles" })

      first_articles_past_24h = Article.where(nth_published_by_author: 1).where("published_at > ?", 1.day).size
    ...
    def get_days_active_past_week_counts
      ...
      record_active_days_of_group(non_new_user_ids, "established")
      record_active_days_of_group(new_user_ids, "new")
    end

    def record_active_days_of_group(user_ids, group)
