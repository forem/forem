  ...
  class EnqueueCustomBatchSendWorker
    ...
    def process_custom_query(email, min_id = nil, max_id = nil)
      ...
      executor.each_id_batch(batch_size: 1000) do |id_batch|
        ...
        eligible_ids = User.email_eligible
          .where(id: filtered_ids)
          .ids
        eligible_ids_set = eligible_ids.to_set

        # Preserve the original order from the custom query results while
        # ensuring we only include IDs that are actually email eligible.
        filtered_user_ids = filtered_ids.select do |candidate_id|
          eligible_ids_set.include?(candidate_id)
        end
