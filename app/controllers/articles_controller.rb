class ArticlesController < ApplicationController
  ...
  def new
    ...
    if @tag.present? && @user&.editor_version == "v2"
      ...
      @article = article_entity.tag_user_editor_v2
    elsif @tag&.submission_template.present? && @user
      authorize Article

      @article = article_entity.tag_user
    elsif @prefill.present? && @user&.editor_version == "v2"
      authorize Article

      @article = article_entity.prefill_user_editor_v2
    elsif @prefill.present? && @user
      authorize Article

      @article = article_entity.prefill_user
    elsif @tag.present?
      skip_authorization

      @article = article_entity.tag
    else
      skip_authorization

      return article_entity.user_editor_v2 if @user&.editor_version == "v2"

      @article = article_entity.user_editor_v1
    end
  end
