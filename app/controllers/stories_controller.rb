class StoriesController < ApplicationController
  ...
  def redirect_to_changed_username_profile
    ...
    if user_or_org.present? && !user_or_org.decorate.fully_banished?
      redirect_to user_or_org.path + internal_nav_param, status: :moved_permanently
    else
      not_found
    end
  end
  ...
  def handle_tag_index
    ...
    if @tag_model.alias_for.present?
      redirect_to "/t/#{@tag_model.alias_for}" + internal_nav_param, status: :moved_permanently
      return
    end
  ...
  def redirect_to_lowercase_username
    ...
    redirect_to "/#{params[:username].downcase}" + internal_nav_param, status: :moved_permanently
  end
