class ApplicationController < ActionController::Base
  ...
  def set_session_domain
    if Rails.env.production?
      ...
      # Check if current request is on a subforum domain
      # NOTE: Subforums must be created in the database for this check to work.
      # If using subforums, ensure Subforem records exist with the domain set correctly.
      is_subforum_domain = Subforem.cached_all_domains.include?(request.host) rescue false
      
      if secondary_domains.include?(request.host)
        request.session_options[:domain] = root_domain(request.host)
      elsif is_subforum_domain
        # For subforum domains, set session domain to the subforum domain itself.
        # This ensures:
        # 1. Email links generated by DeviseMailer on subforum domains work correctly
        # 2. Session cookies match the domain where password reset is being completed
        # 3. CSRF validation passes since cookies and form submission are on same domain
