class FollowsController < ApplicationController
  ...
  def show
    ...
    following_them_check = CheckCachedFollows.call(current_user, params[:followable_type], params[:id])

    return render plain: following_them_check unless params[:followable_type] == "User"

    following_you_check = CheckCachedFollows.call(User.find_by(id: params[:id]), params[:followable_type],
                                                  current_user.id)
  ...
  def bulk_show
    ...
    response = params.require(:ids).map(&:to_i).index_with do |id|
      if current_user.id == id
        ...
        following_them_check = CheckCachedFollows.call(current_user, params[:followable_type], id)
        following_you_check = CheckCachedFollows.call(User.find_by(id: id), params[:followable_type],
                                                      current_user.id)
        if following_them_check && following_you_check
          "mutual"
        elsif following_you_check
