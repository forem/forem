    ...
    doc.css("a").each do |anchor|
      if (article = rich_link_article(anchor))
        ...
        class='chatchannels__richlink'
          ...
          <h4 data-content='sidecar-article'><img src='#{Images::ProfileImage.call(article.cached_user.profile_image_url, length: 90)}' /> #{article.cached_user.name}・#{article.readable_publish_date || 'Draft Post'}</h4>
          </a>".html_safe
        ...
        class='chatchannels__richlink'
          ...
          <h1 data-content='sidecar-user'>
            <img src='#{Images::ProfileImage.call(user.profile_image_url, length: 90)}' data-content='sidecar-user' class='chatchannels__richlinkprofilepic' />
            #{user.name}
          </h1>
          </a>".html_safe
