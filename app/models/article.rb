class Article < ApplicationRecord
  ...
  UNIQUE_URL_ERROR = "This post could not be published because %s has already been taken. " \
  ...
  validates :canonical_url,
            uniqueness: { allow_nil: true, scope: :published,
                          message: format(UNIQUE_URL_ERROR, "Canonical URL") },
            if: :published?
  validates :canonical_url, url: { allow_blank: true, no_local: true, schemes: %w[https http] }
  validates :comments_count, presence: true
  validates :feed_source_url,
            uniqueness: { allow_nil: true, scope: :published, 
                          message: format(UNIQUE_URL_ERROR, "Feed Source URL") },
            if: :published?
