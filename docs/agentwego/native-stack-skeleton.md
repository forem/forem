...
- `services/api/internal/identity` reserves the Ory Kratos-native identity/session/self-service-flow boundary as local DTO/spec types plus a M0-T34 `KratosTargetAdapter`/`NewLocalKratosAdapter` preview spec and M0-T35 `KratosOperationPlan` review-only envelope only; it does not create a Kratos HTTP client, run self-service flows, assert real sessions, or implement custom long-lived auth.
- `services/api/internal/legacyimport` composes Forem article/user clean DTOs with the Ory Kratos `UserIdentityBoundary` through a local-only import bundle, M0-T34 preview service, and M0-T35 `ImportBatchPreview` partial-success service; it emits persistence/search/Kratos preview projections and operation plans but does not execute feed imports, write persistence/search state, or call Kratos.
- M0-T37 adds a native containerization seam: `services/api/Dockerfile` builds only the Go API binary from the repository root, `compose.noema.yml` defines a localhost-only native API/PostgreSQL/Redis environment with optional Elasticsearch profile, and `.github/workflows/noema-native.yml` runs Go tests plus compose config and container build checks before any package push. This is still a development/CI artifact; it does not deploy, connect to production, or read real Secrets.
