...
| `task import:identity-preview-test` | Run the M0-T38 legacy user identity-only preview and Kratos `return_to` propagation tests across `identity`, `legacyimport`, and `http`. | Local Go test cache and checked-in fixtures only; never writes DB/search, executes self-service flows, contacts Kratos, or reads credentials. |
...
## M0-T38 Identity-only Preview + Kratos ReturnTo Verification

The feature batch adds `task import:identity-preview-test` and includes it in `task verify:local`:
...
=== RUN   TestPreviewServiceBuildsLocalImportPlanFromFixture
--- PASS: TestPreviewServiceBuildsLocalImportPlanFromFixture
=== RUN   TestPreviewServiceBuildsIdentityOnlyPreviewWithReturnToOperationPlans
--- PASS: TestPreviewServiceBuildsIdentityOnlyPreviewWithReturnToOperationPlans
=== RUN   TestRouterLegacyImportIdentityPreviewBuildsLocalPlanWithoutExternalDependencies
--- PASS: TestRouterLegacyImportIdentityPreviewBuildsLocalPlanWithoutExternalDependencies
PASS
```

The API smoke path now verifies `POST /legacy-import/identity-preview` and expects `schema_version = noema.legacy-import.identity-preview/v1`, `side_effects = none-local-preview-only`, clean user DTO output, Ory Kratos identity/session/self-service flow preview data, encoded self-service `request_url`, and review-only `operation_plans` preserving `kratos_return_to` as `return_to`. This endpoint is still a local test/preview entry: no production DB, real Secret, S3, Elasticsearch/OpenSearch, live Kratos, self-service flow execution, session cookies/tokens, deploy, or irreversible mutation.
