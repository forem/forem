    ...
    module ActiveRecord
      class Context < ::Ransack::Context
        ...
        def build_association(name, parent = @base, klass = nil)
          ...
          @tables_pot[found_association] = found_association.reflection.chain.map { |reflection|
            alias_tracker.aliased_table_for(reflection.klass.arel_table) do
              root = reflection == found_association.reflection
              name = reflection.alias_candidate(parent.table_name)
              root ? name : "#{name}_join"
            end
          }
          found_association.table = @tables_pot[found_association].first
